...
class UserStatusController extends Controller
{
    ...
    public function store(UserStatusRequest $userStatusRequest)
    {
        // Capturar possíveis exceções durante a execução.
        try {
            // Cadastrar no banco de dados na tabela status
            $userStatus = UserStatus::create([
                'name' => $userStatusRequest->name,
            ]);
            ...
            return redirect()->route('user_statuses.index', ['userStatus' => $userStatus->id])->with('success', 'Status para usuário cadastrado com sucesso!');
        } catch (Exception $exception) {
            // Salvar log
            Log::notice('Status para usuário não cadastrado.', ['error' => $exception->getMessage(), 'action_user_id' => Auth::id()]);
    ...
    public function update(UserStatusRequest $userStatusRequest, UserStatus $userStatus)
    {
        // Capturar possíveis exceções durante a execução.
        try {
            ...
            // Redirecionar o usuário, enviar a mensagem de sucesso
            return redirect()->route('user_statuses.index', ['userStatus' => $userStatus->id])->with('success', 'Status para usuário editado com sucesso!');
        } catch (Exception $exception) {
            // Salvar log
            Log::notice('Status para usuário não editado.', ['error' => $exception->getMessage(), 'action_user_id' => Auth::id()]);
    ...
    public function destroy(UserStatus $userStatus)
    {
        ...
        try {
            ...
            // Salvar log
            Log::notice('Status para usuário não apagado.', ['error' => $exception->getMessage(), 'action_user_id' => Auth::id()]);
